...
Algorithm performance over all agregation scenarios is summarized in Figure ref:figOverAllRes by subset of results at the extremes of both power and temporal aggregation. Each bar represents an algorithms average MNRMSE for a given combination of temporal and power aggregation.
#+begin_quote
...
Sweeps of aggregation level are shown in Figure ref:figIncreaseingAggOverAllRes with a increasing aggregation of time or power plotted dependant variable while the other type is kept constant (CO and FHMM omitted due to poor performance).
#+NAME: figIncreaseingAggOverAllRes
#+begin_quote
/Average MNRMSE performance per algorithm at increasing power (leftmost) and increasing temporal aggregation levels (right three)./ 
#+end_quote
...
- Power aggregation seems to pose the greatest overall impact overall though it is noted that at medium power aggregation the transition between high and low temporal aggregation is very substantial
