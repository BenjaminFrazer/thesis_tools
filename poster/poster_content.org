...
Sweeps of aggregation levels are shown in Figure ref:figIncreaseingAggOverAllRes with a varying degrees of a given aggregation type plotted as the dependant variable while the other type is kept at a constant value (CO and FHMM omitted due to poor performance).
