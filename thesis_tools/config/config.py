...
class ResultsOverall :
    def __init__(self, paths=Paths()):
        self.paths=paths
        self._fileList = self.paths.res_overall_temp_agg_at_all_pows
    def __getitem__(self, item):
        df = pd.read_csv(self._fileList[item-1]) # we index from 1
        df.set_index(["Household","AggLevel","Metric"],inplace=True)
        return df
